...
class account extends db_connect
{
    ...
    public function signup($username, $fullname, $password, $email, $sex, $year, $month, $day, $u_age, $u_sex_orientation, $language = '')
    {
        ...
        $stmt = $this->db->prepare("INSERT INTO users (free_messages_count, language, admob, state, login, fullname, passw, email, salt, balance, bYear, bMonth, bDay, sex, u_age, sex_orientation, regtime, ip_addr) value (:free_messages_count, :language, :admob, :state, :username, :fullname, :password, :email, :salt, :balance, :bYear, :bMonth, :bDay, :sex, :age, :sex_orientation, :createAt, :ip_addr)");
        $stmt->bindParam(":free_messages_count", $default_free_messages_count, PDO::PARAM_INT);
        $stmt->bindParam(":language", $default_user_language, PDO::PARAM_INT);
    ...
    public function updateCounters()
    {
        ...
        $profile = new profile($this->db, $this->id);
        $profile_info = $profile->getVeryShort();
        unset($profile);
        ...
        $stmt->bindParam(":accountId", $this->id, PDO::PARAM_INT);
        $stmt->bindParam(":photos_count", $profile_info['photosCount'], PDO::PARAM_INT);
        $stmt->bindParam(":gifts_count", $profile_info['giftsCount'], PDO::PARAM_INT);
        $stmt->bindParam(":likes_count", $profile_info['likesCount'], PDO::PARAM_INT);
        $stmt->bindParam(":friends_count", $profile_info['friendsCount'], PDO::PARAM_INT);
        $stmt->bindParam(":matches_count", $profile_info['matchesCount'], PDO::PARAM_INT);
    ...
    public function setAllowMatchesGCM($allowLikesGCM)
    {
        $stmt = $this->db->prepare("UPDATE users SET allowMatchesGCM = (:allowMatchesGCM) WHERE id = (:accountId)");
        $stmt->bindParam(":accountId", $this->id, PDO::PARAM_INT);
        $stmt->bindParam(":allowMatchesGCM", $allowLikesGCM, PDO::PARAM_INT);
        $stmt->execute();
    }

    public function getAllowMatchesGCM()
    {
        $stmt = $this->db->prepare("SELECT allowMatchesGCM FROM users WHERE id = (:id) LIMIT 1");
        $stmt->bindParam(":id", $this->id, PDO::PARAM_INT);
    ...
    public function get()
    {
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $matches = new matches($this->db, $this->id);
                $matches->setRequestFrom($this->id);

                $matches_count = $matches->getNewCount($row['last_matches_view']);
