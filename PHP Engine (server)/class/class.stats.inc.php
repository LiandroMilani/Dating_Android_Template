...
class stats extends db_connect
{
    ...
    public function getModeratedPhotosCount()
    {
        $stmt = $this->db->prepare("SELECT count(*) FROM photos WHERE moderateAt > 0 AND removeAt = 0");
        $stmt->execute();

        return $number_of_rows = $stmt->fetchColumn();
    }

    public function getUnModeratedPhotosCount()
    {
        $stmt = $this->db->prepare("SELECT count(*) FROM photos WHERE moderateAt = 0 AND removeAt = 0");
        $stmt->execute();
    ...
    public function getModeratedAccountsCount()
    {
        $stmt = $this->db->prepare("SELECT count(*) FROM users WHERE accountModerateAt > 0 AND accountRejectModerateAt = 0");
        $stmt->execute();

        return $number_of_rows = $stmt->fetchColumn();
    }

    public function getUnModeratedAccountsCount()
    {
        $stmt = $this->db->prepare("SELECT count(*) FROM users WHERE accountModerateAt = 0 AND accountRejectModerateAt = 0");
        $stmt->execute();
