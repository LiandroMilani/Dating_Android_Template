...
class photos extends db_connect
{
    ...
    public function remove($photoId)
    {
        ...
        if ($photoInfo['fromUserId'] != $this->getRequestFrom()) {
    ...
    public function approve($itemId)
    {
        ...
        if ($stmt->execute()) {
            ...
            $gcm->setData(GCM_NOTIFY_MEDIA_APPROVE, "You gallery item is approved.", $itemId);
            $gcm->send();

            $notify = new notify($this->db);
            $notify->createNotify($itemInfo['fromUserId'], 0, NOTIFY_TYPE_MEDIA_APPROVE, $itemId);
            unset($notify);
        }
    ...
    public function reject($itemId)
    {
        ...
        $gcm = new gcm($this->db, $itemInfo['fromUserId']);
        $gcm->setData(GCM_NOTIFY_MEDIA_REJECT, "You gallery item is rejected.", $itemId);
        $gcm->send();

        $notify = new notify($this->db);
        $notify->createNotify($itemInfo['fromUserId'], 0, NOTIFY_TYPE_MEDIA_REJECT, $itemId);
        unset($notify);
    ...
    public function get($profileId, $photoId = 0, $accessMode = 0)
    {
        ...
        if ($accessMode == 0) {
            ...
            $stmt = $this->db->prepare("SELECT id FROM photos WHERE accessMode = 0 AND fromUserId = (:fromUserId) AND moderateAt > 0 AND removeAt = 0 AND id < (:photoId) ORDER BY id DESC LIMIT 20");
            ...
            if ($this->getRequestFrom() == $profileId) {
                ...
                $stmt = $this->db->prepare("SELECT id FROM photos WHERE fromUserId = (:fromUserId) AND moderateAt > 0 AND removeAt = 0 AND id < (:photoId) ORDER BY id DESC LIMIT 20");
                $stmt->bindParam(':fromUserId', $profileId, PDO::PARAM_INT);
                $stmt->bindParam(':photoId', $photoId, PDO::PARAM_INT);
            }
        }
