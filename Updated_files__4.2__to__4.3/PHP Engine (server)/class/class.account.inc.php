...
class account extends db_connect
{
    ...
    public function signup($username, $fullname, $password, $email, $sex, $year, $month, $day, $u_age, $u_sex_orientation, $language = '')
    {
        ...
        $stmt = $this->db->prepare("INSERT INTO users (free_messages_count, language, admob, state, login, fullname, passw, email, salt, balance, bYear, bMonth, bDay, sex, u_age, sex_orientation, regtime, accountModerateAt, accountPostModerateAt, ip_addr) value (:free_messages_count, :language, :admob, :state, :username, :fullname, :password, :email, :salt, :balance, :bYear, :bMonth, :bDay, :sex, :age, :sex_orientation, :createAt, :accountModerateAt, :accountPostModerateAt, :ip_addr)");
        ...
        if ($stmt->execute()) {
            ...
            if ($this->auto_moderate) {

                $moderator = new moderator($this->db);
                $moderator->approve($this->getId());
                unset($moderator);
            }
    ...
    public function setPhoto($array_data)
    {
        ...
        if ($stmt->execute()) {
            ...
            if ($info['accountRejectModerateAt'] != 0) {

                $moderator = new moderator($this->db);
                $moderator->post($this->getId());
                unset($moderator);
            }
        }
    }
