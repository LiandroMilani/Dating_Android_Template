...
class search extends db_connect
{
    ...
    private function getCount($queryText, $gender = -1, $online = -1, $photo = -1, $ageFrom = 13, $ageTo = 110)
    {
        ...
        $sql = "SELECT count(*) FROM users WHERE state = 0 AND (login LIKE '{$queryText}' OR fullname LIKE '{$queryText}' OR email LIKE '{$queryText}' OR country LIKE '{$queryText}')".$genderSql.$onlineSql.$photoSql.$dateSql;
    ...
    public function query($queryText = '', $userId = 0, $gender = -1, $online = -1, $photo = -1, $ageFrom = 13, $ageTo = 110)
    {
        ...
        $sql = "SELECT id, regtime FROM users WHERE state = 0 AND (login LIKE '{$queryText}' OR fullname LIKE '{$queryText}' OR email LIKE '{$queryText}' OR country LIKE '{$queryText}') AND id < {$userId}".$genderSql.$onlineSql.$photoSql.$dateSql.$endSql;
        $stmt = $this->db->prepare($sql);
