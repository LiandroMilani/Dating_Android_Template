    ...
    if (!auth::isSession() && isset($_COOKIE['user_name']) && isset($_COOKIE['user_password'])) {
        ...
        if ($accountInfo['error'] === false && $accountInfo['state'] == ACCOUNT_STATE_ENABLED) {
            ...
            if ($auth->authorize($accountInfo['id'], $_COOKIE['user_password'])) {
                ...
                auth::setSession($accountInfo['id'], $accountInfo['username'], $accountInfo['fullname'], $accountInfo['lowPhotoUrl'], $accountInfo['verify'], $accountInfo['pro'], $account->getAccessLevel($accountInfo['id']), $_COOKIE['user_password']);
